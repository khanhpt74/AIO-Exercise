def MAE(num_1,num_2,num_3):
   return (1/num_1) * abs(num_2 - num_3)

def MSE(num_1,num_2,num_3):
...
def use_name_root_error(number, names):
   ...
   if activation_function_name_v1(names) and tests_isnumeric(number):
      for _ in range(number -1):
        ...
        if names == "MAE":
           loss = MAE(number,target,predict)
           total += loss
           print(f"loss name: {names}, vậy giá trị predict: {predict}, vậy giá trị target: {target}, vậy kết quả loss: {loss}")
        elif names == "MSE":
           loss = MSE(number,target,predict)
           total = total + loss
           print(f"loss name: {names}, vậy giá trị predict: {predict}, vậy giá trị target: {target}, vậy kết quả loss: {loss}")
